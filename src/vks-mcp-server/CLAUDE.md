...
Node-group write DTOs (validate a create body with `validate_nodegroup_create` before calling `create_nodegroup`: local name/bounds rules + cached discovery cross-checks — subnet in the cluster's VPC AND has secondary subnets, secondarySubnets = the chosen subnet's `secondary_subnets` CIDRs verbatim, flavorId/diskType in the subnet's zone, sshKeyId/securityGroups in the region):

- **Create** (`CreateNodeGroupDto`/`NodeGroupSpec`): required `name`, `flavorId`, `diskType` (a volume-type **ID** from `list_volume_types`), `sshKeyId`, `diskSize`, `numNodes`, `subnetId` (must be a subnet that HAS secondary subnets), `secondarySubnets` (that subnet's `secondary_subnets` CIDRs copied verbatim from `list_subnets`, non-empty — derived, never asked). Optional: `os` (ubuntu/linux/rocky, top level — NOT inside `upgradeConfig`), `enablePrivateNodes`, `enabledEncryptionVolume`, `securityGroups`, `upgradeConfig`, `labels`, `taints`, `tags`, `autoScaleConfig`, `placementGroupConfigDto`.
