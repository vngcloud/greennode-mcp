...
Node-group write DTOs (validate a create body with `validate_nodegroup_create` before calling `create_nodegroup`: local name/bounds rules + cached discovery cross-checks — subnet in the cluster's VPC; secondarySubnets scoped by the cluster's networkType (CILIUM_NATIVE_ROUTING: the chosen subnet's `secondary_subnets` CIDRs verbatim, non-empty — the subnet must have them; otherwise `[]`); flavorId/diskType in the subnet's zone; sshKeyId/securityGroups in the region):

- **Create** (`CreateNodeGroupDto`/`NodeGroupSpec`): required `name`, `flavorId`, `diskType` (a volume-type **ID** from `list_volume_types`), `sshKeyId`, `diskSize`, `numNodes`, `subnetId`, `secondarySubnets` (derived, never asked — CILIUM_NATIVE_ROUTING cluster: the chosen subnet's `secondary_subnets` CIDRs copied verbatim from `list_subnets`, non-empty, so the subnet must HAVE them; other networkTypes: `[]`). Optional: `os` (ubuntu/linux/rocky, top level — NOT inside `upgradeConfig`), `enablePrivateNodes`, `enabledEncryptionVolume`, `securityGroups`, `upgradeConfig`, `labels`, `taints`, `tags`, `autoScaleConfig`, `placementGroupConfigDto`.
