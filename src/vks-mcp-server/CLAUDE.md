...
- **The kubeconfig endpoint returns a JSON envelope** `{kubeConfig, status, ...}` (was bare YAML); a new cluster has NO kubeconfig until `generate_kubeconfig` mints one (async, `POST .../kubeconfig`). `kubeconfig.extract_kubeconfig` handles both shapes. `expiration_days` (1-1825) has **no default** — a credential lifetime is a user decision (never-silent-defaults), so the schema forces the ask.
